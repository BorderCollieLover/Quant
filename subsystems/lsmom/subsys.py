...
class Lsmom():
    ...
    def run_simulation(self, historical_data, debug=False, use_disk=False):
        ...
        instruments = self.instruments_config["currencies"] + self.instruments_config["commodities"] + self.instruments_config["metals"]
        ...
        for i in portfolio_df.index:
            ...
            for inst in tradable:
                votes = np.sum([1 for pair in self.pairs if historical_data.loc[date, "{} ema{}".format(inst, str(pair))] > 0]) \
                    + np.sum([-1 for pair in self.pairs if historical_data.loc[date, "{} ema{}".format(inst, str(pair))] < 0])
